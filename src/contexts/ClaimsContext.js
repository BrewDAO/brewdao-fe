...
import brewDaoAbi from '../contracts/BrewDAO.json'

const CONTRACT_ADDRESS = '0x7f906b78d90689fe6746195e87d60632a024f9ba'
const ALCHEMY_API_KEY = '8KNuNbqQnOc-rLF_Ghz1XqWic4DkQbRg'
...
const _getAddressBalance = async address => {
  ...
  const contract = new ethers.Contract(CONTRACT_ADDRESS, brewDaoAbi, provider)
...
const validateETHAddress =
  ...
  async ({ address, data }) => {
    try {
      ...
      if (!hasNotClaimedBeerToday) throw new Error(`Wallet has already claimed a beer today.`)

      // check balance > 100
      const balance = await _getAddressBalance(refinedAddress)
      const hasEnoughTokens = balance >= 100
      if (!hasEnoughTokens) throw new Error(`Wallet does not have > 100 BREW tokens.`)
      dispatch({
